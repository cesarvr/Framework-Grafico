...
zeShader::zeShader(){};
zeShader::zeShader(std::string shader, std::string path){

    cargarShader(shader, path);
...
void zeShader::cargarShader(std::string shader, std::string path){
    ...
    bool link = zeShader::linkarPrograma(ShaderProgram);
    if (!link) {
...
bool zeShader::estaCargadoShader(){ return shaderCargado;  }
